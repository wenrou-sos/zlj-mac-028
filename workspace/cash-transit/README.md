...
| 账号 | 角色 | 登录后能做什么 |
| --- | --- | --- |
| `1001` 张建国 | 调度员 | 全局视角：派车排班、取消/办结、异常处置、车辆送修/复归、人员请假/复岗、总览 |
| `4001` 周金库 | 金库管理员（中心金库） | 只看本金库任务，办理本金库出库/回库交接 |
| `4002` 吴库管 | 金库管理员（朝阳金库） | 同上，限朝阳金库 |
| `2001` 王海涛 | 车长 | 只看本人任务：出发、到离站、网点交接、上报异常 |
| `2004` 赵磊 | 车长（演示任务003） | 处理异常挂起任务的国贸站交接 |
| `3001` 吴铁军 | 驾驶员 | 只读本人任务，无任何操作入口 |
| `5003` 蒋帆 | 中关村柜员 | 只看本网点任务与交接，办理本站接收/移交 |
| `admin` | 系统管理员 | 等同调度员 + Django 后台（同密码） |

#### 岗位权限矩阵（后端强制 + 菜单按钮级控制）

| 操作 | 调度员 | 金库管理员 | 车长 | 押运员 | 驾驶员 | 网点柜员 |
| --- | :-: | :-: | :-: | :-: | :-: | :-: |
| 派车排班 / 取消 / 办结 | ✅ | | | | | |
| 车辆送修·复归、人员请假·复岗 | ✅ | | | | | |
| 异常处置 | ✅ | | | | | |
| 出发、到离站、办结站点 | | | 本任务 | | | |
| 金库出库 / 回库 | | 本金库 | | | | |
| 网点接收 / 移交 | | | 本任务车组 | 本任务车组 | | 本网点 |
| 异常上报 | ✅ | | 本任务车组 | 本任务车组 | | |
| 数据范围 | 全部 | 本金库出入任务 | 本人任务 | 本人任务 | 本人任务(只读) | 本网点任务 |

越权请求一律返回 **403** 并说明原因，例如：
`「派车排班」仅由调度员办理；您当前岗位为「驾驶员」，无权执行该操作`。
交接弹窗的交出人/接收人改由
`GET /api/tasks/{id}/handover-candidates/?phase=…&stop_id=…`
按环节返回（金库管理员限本金库、车组限本任务、柜员限本站），
从源头杜绝选错岗位的人办交接。
...
| 交接候选人 | `GET /api/tasks/{id}/handover-candidates/?phase=&stop_id=`（按岗位返回交出/接收人） |
...
按日期判定排班占用、跨日期取消/办结不误置车辆待命、
**六类岗位的动作级越权拦截（403）与任务/交接/异常数据范围隔离**等 88 个用例。
