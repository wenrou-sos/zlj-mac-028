...
- **车辆**：5 辆运钞车。每日排班前在「车辆·款箱·人员」页**登记送修 / 复归待命**：
  - 维修中的车不能排班，下拉中禁用并注明"维修中"；
  - 执行未完成任务（含已派车、押运中、挂起）的车**不允许送修**，
    必须先办结或取消任务；
  - 每次状态变更记录操作人、时间、事由（"变更记录"抽屉可查），
    派车出发、归队、取消也自动留痕。
- **人员**：调度、车长、武装押运员、驾驶员、金库管理员、网点柜员。
  每日排班前**登记请假（休假 / 培训 / 休息 / 其他）/ 复岗**：
  - 缺勤人员**不能排进车组**（下拉禁用并注明缺勤类型）、
    **不能办理款箱交接**（休假柜员无法接收/下单，后端强校验）；
  - 有未完成任务在身的人员不能请假，需先调整车组或办结任务；
  - 同一人不能同时进入两个未完成任务的车组；请/复岗全程留痕。
- **款箱**：柜员尾箱、现金箱、凭证箱、ATM 加钞箱，含面额合计。
- 排班下拉对每个不可用资源给出**明确冲突原因**，例如
  "车辆 京A·Y8002 当天已执行任务 YY-…（押运中）"、
  "孙鹏 今日培训，不能排班"、"王海涛 已被任务 YY-… 占用"。
  冲突按**所选计划日期**判定：明天有任务的车和人今天仍可排班，
  排班日期变更后下拉候选自动刷新并清空当日失效的已选项。
- 车辆实体状态在任务取消 / 办结时按车上**剩余任务**同步：
  一辆车跨日期有多个任务时，取消或办结其中一个，只要还有在途/挂起任务就保持
  「执行任务」，全部结束后才归队「待命」（状态变更仍逐笔留痕）。
...
| 车辆 | `POST /api/vehicles/{id}/repair/`（送修，body: reason）、`.../return-service/`（复归）、`GET .../status-logs/` |
| 人员 | `POST /api/staff/{id}/leave/`（请假，body: leave_type/reason）、`.../return-duty/`（复岗）、`GET .../duty-logs/` |

车辆、人员列表中的 `schedulable` / `unavailable_reason` / `busy_task_no(s)` 字段
供排班页面判断可选性与展示冲突原因；传 `date=YYYY-MM-DD` 查询参数时，
占用按该日期计算（不传则为全局占用，供资源维护页使用）。
...
错误封签拦截、交接人员必填且必须在岗、验证码核对、
上收"移交→回库"全链路自动办结与款箱日循环、占用款箱/车辆/人员冲突派车拦截、
车辆送修复归与占用保护、人员请假复岗与占用保护、全量状态变更留痕、
按日期判定排班占用、跨日期取消/办结不误置车辆待命等 57 个用例。
